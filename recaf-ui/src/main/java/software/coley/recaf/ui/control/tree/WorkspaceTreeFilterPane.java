...
public class WorkspaceTreeFilterPane extends BorderPane {
     ...
     * @param hidePanelAction Action to hide the workspace panel, or {@code null} to omit the button.
     */
    public WorkspaceTreeFilterPane(@Nonnull WorkspaceTree tree, @jakarta.annotation.Nullable Runnable hidePanelAction) {
        ...
        collapseAll.setOnAction(_ -> software.coley.recaf.ui.control.tree.TreeItems.recurseClose(tree, tree.getRoot()));
        ...
        if (hidePanelAction != null) {
            javafx.scene.control.Button hidePanel = new javafx.scene.control.Button();
            hidePanel.setGraphic(new FontIconView(CarbonIcons.OPEN_PANEL_FILLED_LEFT));
            javafx.scene.control.Tooltip hideTooltip = new javafx.scene.control.Tooltip();
            hideTooltip.textProperty().bind(Lang.getBinding("menu.file.toggle-panel"));
            hidePanel.setTooltip(hideTooltip);
            hidePanel.getStyleClass().addAll(Styles.BUTTON_ICON, Styles.ACCENT, Styles.FLAT, Styles.SMALL);
            hidePanel.setOnAction(_ -> hidePanelAction.run());
            rightContainer.getChildren().addFirst(hidePanel);
        }
        ...
        textField.textProperty().addListener((_, _, _) -> update(tree));
        caseSensitivity.addListener((_, _, _) -> update(tree));
    }
    ...
    private boolean match(@Nonnull TreeItem<PathNode<?>> item) {
        ...
        return caseSensitivity.get() ? path.contains(textField.getText()) : path.toLowerCase()
