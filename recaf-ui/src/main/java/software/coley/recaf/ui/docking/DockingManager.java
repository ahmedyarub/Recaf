...
public class DockingManager {
    ...
    private final Bento bento = new Bento() {
        ...
        protected StageBuilding newStageBuilding() {
            return new StageBuilding(this) {
                @Override
                protected void initializeFromSource(@Nonnull Scene sourceScene, @Nonnull Scene newScene, @Nullable Stage sourceStage, @Nonnull DragDropStage newStage, boolean sourceIsOwner) {
        ...
        protected DockableDragDropBehavior newDragDropBehavior() {
            return new DockableDragDropBehavior() {
                @Override
                public boolean canReceiveDockable(@Nonnull DockContainerLeaf targetContainer, @Nullable Side targetSide, @Nonnull Dockable dockable) {
                    // If we see the 'never receive' ID in a container then we bail.
                    if (targetContainer.getDockables()
                            .stream()
                            .anyMatch(d -> d.getDragGroupMask() == GROUP_NEVER_RECEIVE))
                        return false;
    ...
    @Inject
    public DockingManager(@Nonnull WorkspaceManager workspaceManager, @Nonnull WindowManager windowManager, @Nonnull Actions actions, @Nonnull Instance<LoggingPane> loggingPaneProvider, @Nonnull Instance<WelcomePane> welcomePaneProvider, @Nonnull Instance<WorkspaceExplorerPane> workspaceExplorerProvider, @Nonnull ResourceSummaryServiceConfig resourceSummaryConfig) {
        ...
        bento.stageBuilding().setStageFactory(_ -> {
            DragDropStage stage = new DragDropStage(true);
            windowManager.registerAnonymous(stage);
            return stage;
        });
        bento.stageBuilding().setSceneFactory((_, content, width, height) -> {
    ...
    private static void addSideOptions(@Nonnull ContextMenu menu, @Nonnull DockContainerLeaf container) {
        for (Side side : Side.values()) {
            ...
            MenuItem item = new ActionMenuItem(Lang.getBinding("misc.direction." + side.name()
                    .toLowerCase()), graphic, () -> container.sideProperty().set(side));
    ...
    /**
     * Toggles the visibility of the workspace tools panel (left side tree).
     */
    public void toggleWorkspacePanel() {
        var containerPath = bento.search().container(ID_CONTAINER_WORKSPACE_TOOLS);
        if (containerPath == null)
            return;
        software.coley.bentofx.layout.DockContainer container = containerPath.tailContainer();
        if (container instanceof software.coley.bentofx.layout.container.DockContainerLeaf leaf) {
            javafx.scene.Node current = leaf.getParent();
            while (current != null) {
                if (current instanceof software.coley.bentofx.layout.container.DockContainerBranch branch && branch.getItems()
                        .contains(leaf)) {
                    branch.setContainerCollapsed(leaf, !leaf.isCollapsed());
                    return;
                }
     ...
     *
     * @param translationKey Dockable title translation key.
     * @param icon           Dockable icon.
     * @param content        Dockable content to display.
    ...
    public Dockable newToolDockable(@Nonnull String translationKey, @Nonnull Ikon icon, @Nonnull Node content) {
        return newToolDockable(translationKey, _ -> new FontIconView(icon), content);
    }
     ...
     *
     * @param translationKey Dockable title translation key.
     * @param iconFactory    Dockable icon factory.
     * @param content        Dockable content to display.
     * @return Created dockable.
     */
    @Nonnull
    ...
    public Dockable newTranslatableDockable(@Nonnull String translationKey, @Nonnull Ikon icon, @Nonnull Node content) {
        return newTranslatableDockable(translationKey, _ -> new FontIconView(icon), content);
    }
     ...
     * Creates a {@link Dockable}.
     *
     * @param titleBinding Dockable title translation binding.
     * @param iconFactory  Dockable icon factory.
     * @param content      Dockable content to display.
     * @return Created dockable.
     */
    ...
    /**
     * @param id ID of the container to create.
    ...
    private DockContainerBranch newWorkspaceContainer() {
        ...
        explorer.setSide(javafx.geometry.Side.LEFT);
     ...
     *
     * @param workspace Workspace to register the close listener for.
     * @param explorer  Workspace explorer pane to destroy when the workspace is closed.
     */
    ...
    private static class ClippedHeaders extends Headers {
         ...
         * @param orientation Which axis to layout children on.
         * @param side        Side in the parent container where tabs are displayed.
         */
    ...
    private class SummaryDisplayListener implements WorkspaceOpenListener {
        ...
        public void onWorkspaceOpened(@Nonnull Workspace workspace) {
            ...
            FxThreadUtil.run(() -> {
                if (bento.search()
                        .replaceContainer(ID_CONTAINER_ROOT_TOP, DockingManager.this::newWorkspaceContainer)) {
